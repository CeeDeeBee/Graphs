...
1. To create 100 users with an average of 10 friends each, how many times would you need to call `add_friendship()`? Why?
   In the algorithm the number of friendships is equal to num users \* avg friends // 2 which here comes out to 500. Therefore add_friendship() would be called 500 times.

2. If you create 1000 users with an average of 5 random friends each, what percentage of other users will be in a particular user's extended social network? What is the average degree of separation between a user and those in his/her extended network?
   I would expect the average degree of separation between users to be around 6 based on the six degrees of separation principal for large networks. Additionally I would expect the percentage of users to be in the eventual network to be close to 100%.
...
1. You might have found the results from question #2 above to be surprising. Would you expect results like this in real life? If not, what are some ways you could improve your friendship distribution model for more realistic results?
   I would expect this phenomon to continue for larger networks. I looked into the problem a bit and found that the six degrees of separaiton principal held even for a study done with a network of 240 million people.
